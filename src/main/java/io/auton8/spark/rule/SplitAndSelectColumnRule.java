...
public class SplitAndSelectColumnRule implements IRule {
	...
	public boolean canApply(Dataset<Row> df, Map<String, Object> params) throws RuleNotApplicatbleException {
		...
		if (!params.containsKey("separator"))
			throw new RuleNotApplicatbleException("separator parameter is required");
		if (!params.containsKey("index"))
			throw new RuleNotApplicatbleException("index parameter is required");
		return true;
	}
	...
	public Dataset<Row> apply(Dataset<Row> df, Map<String, Object> params) throws RuleNotApplicatbleException {
		if (canApply(df, params)) {
			...
			Integer index = ((Double) params.get("index")).intValue();
			String separator = (String) params.get("separator");


			return df.withColumn(aliasColumn, split(col(normalizeColumnNameForDF(originalColumn)), separator).getItem(index));
		}
		return null;
	}
