...
//		return df;
	...
	default Dataset<Row> comparisonRule(Dataset<Row> df,Map<String, Object> params , List<String> cols){
		...
		try {
		df = df.withColumn(compColumn, when(not(df.col(normalizedOriginalColumn).eqNullSafe(df.col(normalizeColumnNameForDF(newColumn)))),"not matched").otherwise("matched"));
		}
		catch(Exception e) {
			e.printStackTrace();
			throw e;
